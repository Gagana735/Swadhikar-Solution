...
import type { ChatMessageV2 } from "@/hooks/use-chat-v2";
...
function renderMarkdown(text: string): string {
  return text
    ...
    .replace(/\*(.+?)\*/g, "<em>$1</em>")
    .replace(/^#{1,3} (.+)$/gm, "<strong class=\"text-base block mt-2 mb-1\">$1</strong>")
...
export function MessageBubbleV2({ message }: MessageBubbleV2Props) {
  ...
  let bubbleStyle = "bg-white border-border text-foreground shadow-sm";
  if (!isUser && message.module) {
    switch (message.module) {
      case "legal":
        bubbleStyle = "bg-blue-50 border-blue-200 text-blue-900 shadow-sm";
        break;
      case "kisan":
        bubbleStyle = "bg-green-50 border-green-200 text-green-900 shadow-sm";
        break;
      case "yojana":
        bubbleStyle = "bg-orange-50 border-orange-200 text-orange-900 shadow-sm";
        break;
    }
  } else if (isUser) {
    bubbleStyle = "bg-[#E0F8D8] border-[#c5e6bc] text-slate-800 shadow-sm";
  }
    ...
    >
      <div className={cn("flex max-w-[88%] md:max-w-[78%]", isUser ? "flex-row-reverse" : "flex-row")}>
        ...
        <div className={cn("flex-shrink-0 flex items-end", isUser ? "ml-2" : "mr-2")}>
          <div className={cn(
            "w-8 h-8 rounded-full flex items-center justify-center shadow-sm text-xs font-bold",
        ...
        <div className={cn(
          ...
          {/* Module badge */}
          {!isUser && message.module && message.module !== "general" && MODULE_LABELS[message.module] && (
              ...
              dangerouslySetInnerHTML={{
                __html:
                  ...
                  (isStreaming && message.message
                    ? '<span class="inline-block w-0.5 h-4 bg-current align-middle ml-0.5 animate-pulse">▋</span>'
                    : ""),
              }}
            />
          )}
